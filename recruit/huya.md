...
- 线程不安全，主要是hash碰撞与扩容导致。
- hash碰撞是指如果有多个线程在某一时刻同时操作HashMap并执行put操作，并且有大于两个的key的hash值相同，那么就有可能多个线程会得到相同的头结点，导致插入新节点时最后一个插入节点覆盖掉前面插入的数据，造成数据丢失。
- 扩容是指当多个线程同时检测到总数量超过门限值的时候就会同时调用resize操作，各自生成新的数组并rehash后赋给该map底层的数组table，但是最终只有最后一个线程生成的新数组被赋给table变量，其他线程的均会丢失。
...
IO通常是指数据在内部存储器和外部存储器或者其他周边设备之间的输入输出。简而言之，从硬盘中读写数据或者从网络上收发数据，都属于IO行为。以数据输入为例，输入操作通常可以分为2个阶段：

1. 等待操作系统内核把数据准备好；
2. 将数据从操作系统内核复制到用户进程空间。

虽然一个IO操作只有2个阶段，但是根据IO是否阻塞，是否同步，可以把IO分为不同的模型。在Unix编程中，一共有5种不同的IO模型：

- 阻塞IO（blocking IO）：所有IO操作（1阶段到2阶段）都是阻塞的。
- 非阻塞IO（nonblocking IO）：准备数据阶段（1阶段）不需要阻塞，但是需要用户进程一直轮循查询数据是否已经准备好；复制数据阶段（2阶段）需要阻塞。
- IO复用（IO multiplexing）：准备数据阶段（1阶段）阻塞，但是不需要等待具体的IO操作，可以同时等待多个文件描述符就绪；复制数据阶段（2阶段）需要阻塞。
- 信号驱动IO（signal driven IO）：准备数据阶段（1阶段）不需要阻塞，数据准备好之后系统内核会发信号通知用户进程；复制数据阶段（2阶段）需要阻塞。
- 异步IO（asynchronous IO）：所有IO操作都不阻塞，IO操作完成时系统内核会通知用户进程。信号驱动IO的是系统内核通知何时开始IO操作，异步IO是系统内核通知何时完成IO操作。
...
1. 继承Thread类，重写run方法；
2. 实现Runnable接口，实现Runnable接口的类的实例对象作为Thread构造函数的target；
3. 使用定时器；
